...
---
# <a name="get-started-with-the-azure-libraries-for-java"></a>開始使用適用於 Java 的 Azure 程式庫

本指南會逐步引導您使用 Azure 服務主體來設定開發環境，並執行程式碼範例來透過適用於 Java 的 Azure 程式庫在 Azure 訂用帳戶中建立及使用資源。
...
- [Java 8](http://www.oracle.com/technetwork/java/javase/downloads/index.html) (包含在 Azure Cloud Shell 內)
...
[使用 Azure CLI 2.0 建立服務主體](/cli/azure/create-an-azure-service-principal-azure-cli)並擷取輸出。 您必須在 password 引數中提供[安全的密碼](https://docs.microsoft.com/azure/active-directory/active-directory-passwords-policy)，而不是提供 `MY_SECURE_PASSWORD`。
...
將此檔案儲存在系統上可供程式碼讀取且安全的位置。 在殼層中使用驗證檔案的完整路徑來設定環境變數 `AZURE_AUTH_LOCATION`。    
...
## <a name="connect-to-a-sql-database"></a>連線到 SQL 資料庫
