...
本指南會逐步引導您設定開發環境和使用適用於 Java 的 Azure 程式庫。 您會建立服務主體來使用 Azure 進行驗證，並執行某些程式碼範例以在訂用帳戶中建立及使用 Azure 資源。 在使用 Azure 開發 Java 時，您可以選擇使用 Eclipse。 只要是具有 Maven 整合的 IDE 都能運作。 或者，如果您不想使用 IDE，則可以使用 Maven 從命令列執行程式碼。
...
- 最新的 [Eclipse](http://www.eclipse.org/downloads/) 穩定版
...
開啟 Eclipse，選取 [檔案] -> [新增]。 在顯示的新視窗中開啟 Maven 資料夾，然後選取 Maven 專案。 

讓下一個畫面保留預設的選取項目，然後選取 [下一步]。 在原型的這個畫面中執行相同動作。

當您到達要求提供 groupID、ArtifactID 等項目的畫面時，輸入 "com.fabrikam" 作為 groupID，並輸入 "AzureApp" 作為 artifactID。

現在，開啟 pom.xml 檔案。 在 `dependencies` 標籤內新增下列程式碼：
...
現在，儲存 pom.xml。 此動作會提示 Eclipse 下載所有指定的相依性。 這可能需要一點時間。
   
## <a name="install-the-azure-toolkit-for-eclipse"></a>安裝 Azure Toolkit for Eclipse

如果您要以程式設計方式部署 Web 應用程式或 API，就必須要有 [Azure 工具組](azure-toolkit-for-eclipse.md)，但此工具組目前並未用於任何其他種類的開發。 以下是安裝程序的摘要。 如需詳細步驟，請瀏覽[安裝 Azure Toolkit for Eclipse](azure-toolkit-for-eclipse.md)。
...
若要建立 `sshkey`，請開啟 Azure Cloud Shell，然後輸入 `ssh-keygen -t rsa -b 2048`。 為檔案命名，然後存取 .public 檔案以取得金鑰 (您會在下列程式碼中用到)，將其全數複製並貼到您的變數 `sshKey` 中。
